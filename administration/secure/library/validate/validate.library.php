<?php
    ...
    class validate
    {
        ...
        //more agressive version of strip_tags
        ...
        //do basic sanitisation immediately
        public function __construct()
        {

            //make post data safer
            if (!empty($_POST)) {
                foreach ($_POST as $k => $v) {
                    ...
                    if(is_string($v)){
                        ...
                        //don't clean tags if wysiwyg field
                        if($k !== 'description' && $k !== 'sample_data' && $k !== 'paid_tier_terms' && $k !== 'free_tier_terms'){

                            //strip any tags and content
                            $v = $this->strip_tags_content($v);

                            //default strip tags, just in case
                            $v = strip_tags($v);

                            //cheeky encoding, no template engine makes encoding on render a PIA
        ...
        public function decode_json($encoded, $array = true)
        {
            ...
            $decoded = (array)json_decode($encoded, $array);
        ...
        //validate dirty matches a regex expression
        public function sanitise_regex($dirty, $pattern)
        {
            if (!preg_match($pattern, $dirty)) {
                return false;
            }
        ...
        */
        ...
        public function check_mime($file, $file_name)
        { //during upload tmp_name lacks extension so allow two params
