<?php
    ...
    class upload
    {
        ...
        public function upload($retain_filename = false)
        {
            ...
            if ($this->check()) {
                //save the file
                $save = $this->save_file($retain_filename);

                $return['boolean'] = $save['boolean'];
                $return['filename'] = $save['filename'];
            }else{
                $return['errors'] = $this->_validation_errors;
            }
        ...
        //save file to server (MODIFIED FOR AWS S3)
        private function save_file($retain_filename)
        {
            ...
            //AWS S3 method added for persistent storage 
            include_once(LOCAL.'secure/class/S3.php');
            $s3 = new S3(IAM_KEY_ID, IAM_KEY_SECRET);
            $res = $s3->putObjectFile($this->_tmp_name, S3_BUCKET, $this->_file['full_path'], S3::ACL_PRIVATE);
            if ($res) {
                $return['boolean'] = true;
                $return['filename'] = $this->_file['filename'];
            }

            return $return;

            /*
                NON-AWS method removed
