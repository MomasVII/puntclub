<?php
    ...
    class sentry
    {
        ...
        public function __construct()
        {
            ...
            $log_data = array(
                'message' => 'OK: Connection accepted.',
                'session_id' => $this->_session_id,
                'request_time' => $_SERVER['REQUEST_TIME'],
                'url' => $this->get_complete_url(),
                'referer' => $referer,
                'end_user_agent' => $_SERVER['HTTP_USER_AGENT'],
                'end_user_ip' => $_SERVER['REMOTE_ADDR'],
            ...
            if ($sanitised_data) {
                ...
                $mysqli_db->insert($this->_log_table, $log_data);
            } else {
        ...
        private function clean_log_table()
        {
            ...
            //delete any records that are more than a few days old
            $ago = strtotime('- 3 days');

            //perform the wipe, no need to listen for response
            $mysqli_db->raw_query('DELETE FROM `'.$this->_log_table.'` WHERE `request_time` < '.$ago, false);
