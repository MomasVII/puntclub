            ...
            if (fileName) {
                ...
                */
                if ( $input.attr("id") == "paidFile" && $('input[name="append_replace"]').length > 0 ) {
					$('input#appendData').parent().find("legend").html("Append/replace existing data *");
    ...
    $('.accordionEnabler').each(function() {
        ...
        if ( $input[0].hasAttribute("data-target") ) {
            ...
            /* Onload Disable the accordion */
				...
				/*	NOTE: Commented out disable payment / remove price fields
				*	trigger as per Andy's instructions this makes the behaviour
				*	of the arrow and checkbox the same.
				*/
