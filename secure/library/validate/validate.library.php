...
class validate
{
        ...
        if ($allow_empty || !empty($data) && !empty($type)) {
            ...
            switch ($type) {
                ...
                case 'float':
                    ...
                    break;
                case 'firebase_id':
                    return $this->sanitise_firebase_id($data);
                    break;
                case 'restricted':
    ...
    */
    ...
    /**
     *  Validates a firebase id (example: ""  )
     *
     * @param $dirty
     * @return boolean
     */
    private function sanitise_firebase_id($dirty)
    {
        //set search pattern for preg_match
        $regexp = '/^([a-zA-Z0-9_\-]{1,20})$/';
        return !!preg_match($regexp, $dirty);//do the match and parse to boolean via "!!"
    }
